...
def checkNodeClass(green, yellow, red, max_score):
    ...
    for key in max_score:
        to_delete = [k for k in max_score if max_score[key][0] == max_score[k][0] and max_score[key][1] > max_score[k][1]]
        for k in to_delete:
            del max_score[k]
            green.remove(k)
        ...
        if keys:
            for key_net in keys:
                ...
                if dist < th_dist:
                    ...
                    tuple_net = [(t[0], t[2], t[4]) for t in neig_net if
                                 (mapping_net[key_net] in (t[0], t[2]))]
                    tuple_cad = [(t[0], t[2], t[4]) for t in neig_cad if
                                 (mapping_cad[key_cad] in (t[0], t[2]))]
